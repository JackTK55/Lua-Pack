...
-------------------- Auto Updater
local sN, sF, vF, cV = GetScriptName(), "https://raw.githubusercontent.com/Zack2kl/Lua-Pack/master/Lua_Pack.lua", "https://raw.githubusercontent.com/Zack2kl/Lua-Pack/master/version.txt", "2.0.1"
...
-------------- Skin Color Changer
local skinComboBox = gui.Combobox(Q, 'vis_skin_color', 'Skin Color', 'White', 'Black')
-------------------------------------------------------------------------------------------------------------------------------------------------------
...
function model_skin_stuff(cmd) if GetLocalPlayer() == nil or skinComboBox:GetValue() == 0 then return end local val = skinComboBox:GetValue() - 1 if client_GetConVar('r_skin') ~= val then client_SetConVar('r_skin', val, true) end end cb_Register("CreateMove", model_skin_stuff)
