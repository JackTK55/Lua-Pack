...
function Disable_FakeAAGhost(UserCMD) if gui_GetValue("vis_fakeghost") ~= 0 then fakeghostval = gui_GetValue("vis_fakeghost") end if not fakeangleghost_air:GetValue() then return end if GetLocalPlayer():GetProp("m_fFlags") == 256 or GetLocalPlayer():GetProp("m_fFlags") == 262 then gui_SetValue("vis_fakeghost", 0) else if not FakeAAGhost2_round_end then gui_SetValue("vis_fakeghost", fakeghostval) end end end cb_Register("CreateMove", Disable_FakeAAGhost)
...
function fix_bombplant(cmd) if not fixbombplant:GetValue() or not GetLocalPlayer():IsAlive() then return end local in_attack = cmd:GetButtons() | (1 << 0) local in_use = cmd:GetButtons() | (1 << 5) if not in_use or not in_attack then return end local weapon = GetLocalPlayer():GetPropEntity('m_hActiveWeapon') if weapon == nil then return end local weapon_class = weapon:GetClass() if weapon_class == 'CC4' and GetLocalPlayer():GetPropInt('m_bInBombZone') == 1 then cmd:SetButtons((1 << 0)) cmd:SetButtons((1 << 5)) end end cb_Register('CreateMove', fix_bombplant)
