...
local scriptFile = "https://raw.githubusercontent.com/Zack2kl/Lua-Pack/master/Lua_Pack";
local versionFile = "https://raw.githubusercontent.com/Zack2kl/Lua-Pack/master/version";
local currentVersion = "1.3.5.8";
...
-------------------- Hit Log
function HitGroup(hitgroup) if hitgroup == 0 then return "body"; elseif hitgroup == 1 then return "head"; elseif hitgroup == 2 then return "chest"; elseif hitgroup == 3 then return "stomach"; elseif hitgroup == 4 then return "left arm"; elseif hitgroup == 5 then return "right arm";  elseif hitgroup == 6 then return "left leg"; elseif hitgroup == 7 then return "right leg"; elseif hitgroup == 10 then return "body"; end end
local draw_hitsay = {};
...
local ind_Attacker = PlayerIndexByUserID(Event:GetInt("attacker"));
...
c_AllowListener("player_hurt"); cb_Register("Draw", "draws hits in top left", hitlog); cb_Register("FireGameEvent", ChatLogger);  
...
if Defuser:GetValue() then client_exec('buy "defuser"', true); end PWb = false; end current_buy = (PrimaryWeapon.. SecondaryWeapon.. armor); client_exec(current_buy, true); buy = false; end end end
c_AllowListener("player_spawn"); cb_Register("FireGameEvent", buy); 
...
elseif drawCrosshair == true and ComboCrosshair:GetValue() == 4 then client_SetConVar("weapon_debug_spread_show", 0, true); gui_SetValue("esp_crosshair", false);
...
elseif DefuseTime == 5 then screenY3 = screenY/2; local godownby2 = (screenY3 / DefuseTime) / get_abs_fps(); draw_Color(0,0,255,255); draw_FilledRect(0, fill2, 10, screenY); draw_Color(0,0,0,100); draw_FilledRect(0, screenY3, 10, screenY); fill2 = fill2 + godownby2; end end end end

-------------------- Bomb Damage
function DrawDamage()
if not Bomb_Damage:GetValue() then return; end
if entities_FindByClass("CPlantedC4")[1] == nil then return; end local Bomb = entities_FindByClass("CPlantedC4")[1];
...
local bombsite = Bomb:GetProp("m_nBombSite") == 0 and "A" or "B"; draw_SetFont(fontz); draw_Color(r, g, b, a); draw_Text(15, 0, bombsite.. " - ".. C4time.."s")
if g_curtime() < Bomb:GetProp("m_flC4Blow") then
if HealthToTake + 0.95 < Player:GetHealth() and HealthToTake - 0 > 0 then draw_Color(255,255,255,255); draw_Text(15, 25, "-"..math_floor(HealthToTake+1));
elseif HealthToTake + 1 >= Player:GetHealth() then draw_Color(255,0,0,255); draw_Text(15, 25, "FATAL"); end end end end
cb_Register("Draw", "draws bomb damage", DrawDamage);
...
cb_Register("Draw", "draws bomb stuff", drawProgress); cb_Register("FireGameEvent", "just event stuff", bomb);
...
function Tracers()
if GetLocalPlayer() == nil then return end
local sX, sY = draw_GetScreenSize(); local lpTeamNum = GetLocalPlayer():GetTeamNumber(); local players = entities_FindByClass("CCSPlayer");
for i = 1, #players do local player = players[i]; local pX, pY, pZ = client_WorldToScreen(player:GetAbsOrigin());
if player:GetTeamNumber() == lpTeamNum then r,g,b,z = 0,0,255,255; else r,g,b,a = 255,0,0,255; end
...
cb_Register("Draw", full_bright);

-------------------- Disable Post Processing
function Dis_PP() if DPP:GetValue() then client_SetConVar("mat_postprocess_enable", 0, true); else client_SetConVar("mat_postprocess_enable", 1, true); end end
cb_Register("Draw", Dis_PP);
...
local wep = GetLocalPlayer():GetPropEntity("m_hActiveWeapon"); local cwep = wep:GetClass();
...
function speclistfix(E) if gui_GetValue("msc_showspec") == 1 then if E:GetName() == "round_start" then client_exec("cl_fullupdate", true); end end end c_AllowListener("round_start"); cb_Register("FireGameEvent", speclistfix)

-------------------- Spectator list
local inbetween = 0; 
function SpecList()
if not SpectatorList:GetValue() or GetLocalPlayer() == nil then return; end local specX, specY = draw_GetScreenSize(); local inbetween = 0; local players = entities.FindByClass("CCSPlayer");
for i = 1, #players do local player = players[i]; local playername = player:GetName();		
if player ~= GetLocalPlayer() and player:GetHealth() <= 0 and player:GetPropEntity("m_hObserverTarget") ~= nil and playername ~= "GOTV" then
local SpectatorTarget = player:GetPropEntity("m_hObserverTarget"); local SpectatorTargetIndex = SpectatorTarget:GetIndex(); local SpectatorTargetName = SpectatorTarget:GetName();
if SpectatorTargetIndex == LocalPlayerIndex() then r,g,b,a = 255,50,50,255; else r,g,b,a = 255,255,255,255; end draw_Color(r,g,b,a); local tW, tH = draw_GetTextSize(playername.. " ➤ ".. SpectatorTargetName); draw_SetFont(fontSL);  draw_TextShadow(specX-50-tW/2, 0+inbetween-tH*1.25, playername.. " ➤ ".. SpectatorTargetName); inbetween = inbetween + 15; end end end  
...
c_AllowListener("round_end"); c_AllowListener("round_start"); cb_Register("FireGameEvent", StealFix); 

-------------------- Show Team Damage
local damagedone, killed = 0, 0
function KillsAndDamage(e)
if e:GetName() == "player_hurt" then if PlayerIndexByUserID(e:GetInt("attacker")) == LocalPlayerIndex() then damagedone = damagedone + e:GetInt("dmg_health"); end end
if e:GetName() == "player_death" then if PlayerIndexByUserID(e:GetInt("attacker")) == LocalPlayerIndex() then killed = killed + 1; end end
if e:GetName() == "player_connect_full" then damagedone, killed = 0, 0; end end
function DrawsTKsDMG() if not TeamDamageShow:GetValue() or GetLocalPlayer() == nil then return; end draw_Color(255,255,255,255); draw_Text(10, 500, "Damage Done: ".. damagedone); draw_Text(10, 510, "Teammates Killed: ".. killed); end
c_AllowListener("player_hurt"); c_AllowListener("player_death"); c_AllowListener("player_connect_full"); 
