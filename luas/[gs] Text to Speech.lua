...
local data = (function()
	local f, data = pcall( readfile, 'csgo/sound/tts/data.json' )

	if not f then
		writefile( 'csgo/sound/tts/data.json', '{}' )
	end

	return f and json.parse(data) or {}
end)()
...
local opts = {
	'Console Input (tts "message")',
	'Game Chat Input (/tts "message")',
	'Show Saved Text-To-Speech',
	'Auto TTS Sent Team Chat',
...
local send_button = ui.new_button( 'misc', 'miscellaneous', 'Send Selected TTS', function()
	local selected = (ui.get(previous_sounds) or -1) + 1
...
local remove_button = ui.new_button( 'misc', 'miscellaneous', 'Remove Selected TTS', function()
	...
	table.remove( descriptions, selected )
	table.remove( data, selected )

	ui.update( previous_sounds, descriptions )
	ui.set( previous_sounds, selected - 2 )
end)
...
local function send_request( text )
	...
	http.get( url, {params = {key=key, lang=lang, c='mp3', f='alaw_22khz_mono', v='Mike', src=text}}, function(success, response)
		if not success or response.status ~= 200 then
			for k, v in pairs( response ) do
				printf( 0, 'ERROR: ', k, ' ', v )
			end

			return printf( 1, 'ERROR: Failed to get TTS' )
		end
